...
C++20 tarjoaa `std::endian`; C++23 tarjoaa `std::byteswap`:
...
Ennen C++23: `htonl`/`ntohl` (POSIX) tai oma `byteswap` `std::endian`-tarkistuksella. Struct `#pragma pack` + suora memcpy on edelleen riski endianin ja paddingin takia — serialisoi kentät eksplisiittisesti.
