...
function persistWeb() {
  updateLocalSave();
  savePlayerSave(session.karma, session.exportDeaths(), quizHistoryState, studyBacklogState, save.progress);
}
...
function snapshot() {
  ...
  const base = {
    ...
    debugCastList: castListEnabledForWeb(),
    castListOpen,
    castList: cast,
...
function handleKey(key) {
  ...
  if (castListEnabledForWeb() && key === "o" && session.screen === "map") {
    castListOpen = true;
    return;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/key") {
    ...
    if (payload.answer !== undefined && session.screen === "story") {
      handleStoryCode(String(payload.answer));
    } else {
      handleKey(String(payload.key ?? "").toLowerCase());
    }
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify(snapshot()));
    return;
