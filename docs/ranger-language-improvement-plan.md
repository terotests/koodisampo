...
> **Note:** This plan is written for the [terotests/Ranger](https://github.com/terotests/Ranger) repository
> (intended location: `PLAN_LANGUAGE_IMPROVEMENTS.md` in the repo root, next to `PLAN_3.md`).
> It lives in the koodisampo repo only because the authoring agent did not have push access to the Ranger repo.
> It is informed by both the Ranger repo itself (README, `ISSUES.md`, `PLAN_3.md`, `ai/GRAMMAR.md`)
> and by real-world usage in koodisampo (~10 700 lines of `.rgr` game logic compiled to ES6 and Kotlin).
...
- [ ] Normalize line endings in the lexer (accept LF, CRLF, CR) and delete the CRLF workaround; add
...
- [ ] Every type error prints **type names**, never enum integers ("Types were 16 vs 10" → "expected
      `buffer`, got `string`").
...
- [ ] **Recursive relative import resolution** (#61): resolve each `Import` relative to the importing file's
      directory; push/pop that directory on the library-path stack while processing that file's own imports.
      This is the single highest-priority fix in this track — it currently blocks multi-directory projects.
...
- [ ] **Traceability**: emit `// <file>:<line>` comments in generated code behind a `-linemap` flag as a
...
1. Recursive import resolution + tests (Track 4, fixes #61).
2. Line-ending normalization in the lexer + LF fixtures in CI (Track 1, kills the #12 bug class).
3. Type-name diagnostics instead of enum integers (Track 3.3) — tiny change, immediate quality-of-life win.
